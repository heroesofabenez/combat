...
/**
 * DefaultInitiativeFormulaParser
 *
 * @author Jakub Konečný
 */
final class DefaultInitiativeFormulaParser implements InitiativeFormulaParser
{
    public function calculateInitiative(Character $character): int
    {
        ...
        /** @var array{0: non-falsy-string, 1: decimal-int-string, 2: decimal-int-string} $dices */
        for ($i = 1; $i <= (int) $dices[1]; $i++) {
            $result += rand(1, (int) $dices[2]);
        }
        preg_match("/\+([0-9]+)\/([0-9]+)/", $formula, $ammendum);
        /** @var array{0: non-falsy-string, 1: decimal-int-string, 2: decimal-int-string} $ammendum */
